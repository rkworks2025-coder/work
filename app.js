(() => {
  ...
  async function getRandomStoredImageData() {
    const keys = await getStoredImages();
  ...
  async function preloadWorkSplash() {
    try {
      // 1. ローカル在庫からランダムに1枚選んで設定しておく（次アプリ用）
      const cachedData = await getRandomStoredImageData();
      if (cachedData) {
        localStorage.setItem("junkai:preloaded_splash_url", cachedData);
      }

      // 2. 測定中の時間を利用して新規画像を収集
      ...
      for (const url of newImages) {
        // 保存ボタンが押されたらループを抜ける（AbortErrorでキャッチ）
      ...
      if (e.name === 'AbortError') {
        console.log("Preload aborted for submission priority.");
      } else {
  ...
  function init(){
    ...
    if(form){
      form.addEventListener('submit', async ev => {
        ...
        // ★重要: 保存ボタン押下時にバックグラウンド通信をすべて強制切断
    ...
    if(backBtn) backBtn.addEventListener('click', () => { 
      // アボートコントローラーを再初期化してプリロードを再開可能にする
